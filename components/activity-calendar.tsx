...
type ActivityType = 'medical' | 'mental' | 'physical'
...
export function ActivityCalendar({ userId, age }: ActivityCalendarProps) {
  ...
  const activityTypes: ActivityType[] = ['medical', 'mental', 'physical']
  ...
  useEffect(() => {
    ...
    const fetchActivities = async () => {
      ...
      days.forEach(day => {
        ...
        const randomActivity = activityOptions[randomType][
          Math.floor(Math.random() * activityOptions[randomType].length)
        ]
        simulatedActivities[format(day, 'yyyy-MM-dd')] = {
          type: randomType,
    ...
    return (
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map(day => {
          ...
          if (activity?.type === 'medical') bgColor = 'bg-blue-200'
          if (activity?.type === 'mental') bgColor = 'bg-green-200'
          if (activity?.type === 'physical') bgColor = 'bg-red-200'

          return (
            <Card
              key={dateString}
              className={`${bgColor} ${isToday ? 'border-2 border-primary' : ''}`}
            >
